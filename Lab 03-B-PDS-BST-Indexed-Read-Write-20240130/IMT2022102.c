...
int pds_load_ndx(){
    ...
    for(int i = 0; i<repo_handle.rec_count; i++){
        ...
        bst_add_node(&repo_handle.ndx_root, ndx->key, &(ndx->offset));
    }
    printf("Just loaded ndx\n");
...
int put_rec_by_key( int key, void *rec ){
    ...
    *offset = ftell(repo_handle.pds_data_fp);
    printf("Offset: %ld\n", *offset);
    if( bst_add_node(&repo_handle.ndx_root, key, offset) == BST_DUP_KEY){
        return PDS_ADD_FAILED;
    }
...
int pds_close(){
    ...
    preorder(repo_handle.ndx_root, repo_handle.pds_ndx_fp);
